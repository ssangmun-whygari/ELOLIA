...
import com.finalProject.model.admin.order.AdminRefundVO;
...
public class OrdersServiceImpl implements OrdersService {
	...
	public boolean modifyCancelStatus(ModifyCancelStatusDTO modifyCancelStatusDTO) {
		...
		log.info("member_id : " + findMemberId);
		try {
	...
	public Map<String, Object> getAllrefund(adminPagingInfoDTO dto) throws Exception {
		...
		List<AdminRefundVO> rawList;
		List<AdminRefundVO> newList = new ArrayList<>();
		...
		pageMap.put("viewPostCntPerPage", pagingInfo.getViewPostCntPerPage());
		try {
			rawList = oDAO.getAllRefund(pageMap);
			log.info("rawList : " + rawList);
//			for (AdminRefundVO entry : rawList) {
//				String cancel_no_str = entry.getCancel_no();
//				// 마지막 , 를 제거
//				if (cancel_no_str != null && cancel_no_str.endsWith(",")) {
//					cancel_no_str = cancel_no_str.substring(0, cancel_no_str.length() - 1);
//				}
//				String[] cancel_nos = cancel_no_str.split(",");
//				if (cancel_nos.length > 1) {
//					for (String cancel_no : cancel_nos) {
//						AdminRefundVO newEntry = entry.clone();
//						newEntry.setCancel_no(cancel_no);
...
//			}
			...
			resultMap.put("refundList", rawList);
			log.info("refundList : " + rawList);
			resultMap.put("PagingInfo", pagingInfo);
