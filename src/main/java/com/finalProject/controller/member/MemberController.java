...
public class MemberController {
	...
	public String login(
		...
		try {
			...
			if (loginMember == null) { // 입력한 아이디 비밀번호에 해당하는 member가 없다면
				System.out.println("로그인 실패");
				return "redirect:/member/viewLogin?status=fail"; // 로그인 실패
			}
			
			session.setAttribute("loginMember", loginMember);
			
			System.out.println("자동 로그인 : " + autologin);
			if (autologin) {
				String code = UUID.randomUUID().toString();
				Cookie cookie = new Cookie("al", code);
				cookie.setMaxAge(60 * 60 * 24 * AUTOLOGIN_DATE);
				cookie.setPath("/"); // 모든 경로에서 사용 가능
				response.addCookie(cookie);
				memberService.setAutoLogin(loginMember.getMember_id(), code, AUTOLOGIN_DATE); // db에 자동로그인 정보 update
			}
...
//					kakao.kakaoLogout((String) ses.getAttribute("accessToken"), request);
	...
	public String kakaoLogin( 	HttpServletResponse response,
		...
		if (ses.getAttribute("redirectURL") != null) {
			result += ses.getAttribute("redirectURL");
	...
	public String kakaoSignUp(
		...
		// 입력받은 생일 형식을 DB에 저장될 형식으로 변경
		if (memberDTO.getBirthday().length() == 0) {
			memberDTO.setBirthday(null);
		} else {
			memberDTO.setBirthday(memberDTO.getBirthday().replace("-", ""));
		}
	...
	// 네이버 로그인(토큰 api)
	@RequestMapping(value = "/naver/redirect")
