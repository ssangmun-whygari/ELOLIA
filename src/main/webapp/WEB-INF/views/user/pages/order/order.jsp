...
<head>
	...
	<script type="text/javascript">
		...
		function showDeliveryList() {
			document.getElementById('getMemberAddress').checked = false;
			...
			$.each(json.couponList, function (index, item) {
				...
				if (item.member == "All") {
					if (item.coupon_dc_type == "R") {
						couponDCText = `\${parseInt(item.coupon_dc_rate * 100)} %`;
		...
		function applyCouponByRate(couponCode, couponNo, couponName, dcRate) {
			...
			let output = `<h3>\${parseInt(dcRate * 100)} %</h3>
		...
		function resetCouponByRate(couponCode, couponNo, couponName, dcRate) {
			let originalPrice = parseInt($.trim($("#totalOriginalPrices").text().replace(" 원", "").replace(/,/g, "")));
			let couponDcPrice = originalPrice * parseFloat(dcRate);
      couponDcPrice = Math.floor((Math.floor(couponDcPrice) / 10)) * 10 // 10원 단위 맞추기
			let output = `<h3>\${parseInt(dcRate * 100)} %</h3>
	...
	<script>
		...
		function getMemberAddress() {
		    ...
		    const checkbox = document.getElementById("getMemberAddress");
		    ...
		    // 체크 상태에 따라 값 설정
		    if (checkbox.checked) {
            if (document.getElementById("detailAddressNew") !== null) {
              document.getElementById("detailAddressNew").removeAttribute('readonly')
            }
		        ...
		        document.getElementById('deliveryAddressHidden').value = "";
            if (document.getElementById("detailAddressNew") !== null) {
              document.getElementById("detailAddressNew").setAttribute('readonly', 'true')
            }
		    }
		}

    $(function() {
      $("#detailAddressNew").on("input change", function() {
        let 멤버상세주소 = $("#orderMember").attr("data-address").split('/')[2];
        if ($(this).val() !== 멤버상세주소) {
          $("#getMemberAddress").prop('checked', false)
        }
      })
    });
...
<body>
				...
				<!--====== 주문 정보 ======-->
					<div class="checkout-steps-form-style-1">
						...
						<!--====== 주문 상품 정보 ======-->
                            ...
                            <!--====== 주문 상품 정보 끝 ======-->
          <div id="orderMember" style="display: none;" data-address="${orderMember.address}"></div> <!--자바스크립트에서 모델 객체의 정보를 안전하게 읽기 위해 만든 div-->
					<c:choose>
										    ...
										    <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab" tabindex="0">
										        <div class="single-form form-default">
									        		<div class="form-check">
										                <div id="getMemberAddressDiv">
										                    <input type="checkbox" class="form-check-input" id="getMemberAddress" onclick="getMemberAddress();">
										                    <label for="getMemberAddress">주문자 정보와 동일</label>
										                </div>
								            		</div>
										            <div class="form-input form addressForm">
										                <h5 class="ordererHeader">주소</h5>
    ...
    <script>
        var oPay = Naver.Pay.create({
             ...
             "chainId": "VmVwazN5RVgwYTN", // chainId
             // "openType" : "popup",
        });
    </script>
  ...
  <script>
	  ...
	  function showOrderModal(message, width=null, height=null, autoHide=true) {
			let dialog = document.querySelector("#orderModal .modal-dialog");
			let modal = document.getElementById("orderModal");
			let modalText = document.querySelector("#orderModal .modal-content .modal-body .modal-text");
			
			modalText.classList.remove("custom-modal-text");
			dialog.classList.remove("custom-modal");
			modal.style.setProperty("--custom-width", null);
			modal.style.setProperty("--custom-height", null);
			
			
			if (width || height) {
				dialog.classList.add("custom-modal");
				modalText.classList.add("custom-modal-text");
			}
			if (width) {
				modal.style.setProperty("--custom-width", width + "px");
			}
		  	if (height) {
				modal.style.setProperty("--custom-height", height + "px");
		  	}
      ...
      console.log("=== validateData 통과, paymentData : ");
      console.log(paymentData);
      console.log("============ paymentData END ============");
      ...
      console.log("orderName : " + orderName)
			...
			$.ajax({
				...
				dataType: "text",
				success : function(response) {
					console.log("/order/payMethod의 response : " + response)
				},
			...
			case "CARD":
				await payment.requestPayment({
					...
					failUrl: window.location.origin + "/pages/order/orderFail", // 결제 요청이 실패하면 리다이렉트되는 URL
					card: {
						useEscrow: false,
						flowMode: "DEFAULT",
						useCardPoint: false,
			...
			case "TRANSFER":
				await payment.requestPayment({
					...
					failUrl: window.location.origin + "/pages/order/orderFail",
					transfer: {
						cashReceipt: {
						type: "소득공제",
						},
	...
	$(function() {
		if (getCookie("hideOrderPageNotice") !== "Y") {
			showOrderPageNoticeModal();
		}
		
		function showOrderPageNoticeModal() {
			let messageHTML = "<h1>안내</h1>";
			messageHTML += "<div style='margin-top: 30px;'>결제 기능은 테스트용으로 개발되어 <strong>결제시 실제로 돈이 빠져나가지 않습니다.</strong></div>"
			messageHTML += "<div style='margin-top: 10px;'>네이버페이 결제 테스트 방법</div>"
			messageHTML += "<div style='margin-top: 10px; font-size: 18px'>'카드 추가하기' 메뉴를 누른 후 더미 카드 정보 입력</div>"
			messageHTML += "<div style='margin-top: 10px; font-size: 18px'>카드번호 : 1111-1111-1111-1111 / 유효기간 : 1111 / CVC : 111 / 카드 비밀번호 : 11 / 카드사 : 자유 선택 </div>"
			showOrderModal(messageHTML, 720, 480, false);
		}
	});
	</script>
</body>
